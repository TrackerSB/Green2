...
public class HeaderReader {
     ...
     * <br><br><b>Note:</b> This method does not read local file header
     * information
     *
    ...
    public ZipModel readAllHeaders(String fileNameCharset) throws ZipException {
        ...
        //Donot read local headers now.
        //zipModel.setLocalFileHeaderList(readLocalFileHeaders());
        return zipModel;
    }
    ...
    private CentralDirectory readCentralDirectory() throws ZipException {
        ...
        try {
            ...
            long offSetStartCentralDir
                    = endCentralDirRecord.getOffsetOfStartOfCentralDir();
            int centralDirEntryCount
                    = endCentralDirRecord.getTotNoOfEntriesInCentralDir();

            if (zipModel.isZip64Format()) {
                offSetStartCentralDir = zipModel.getZip64EndCentralDirRecord()
                        .getOffsetStartCenDirWRTStartDiskNo();
                centralDirEntryCount = (int) zipModel
            ...
            for (int i = 0; i < centralDirEntryCount; i++) {
                ...
                if (signature != InternalZipConstants.CENSIG) {
                    throw new ZipException("Expected central directory entry "
                            + "not found (#" + (i + 1) + ")");
                }
                ...
                //version made by
                readIntoBuff(zip4jRaf, shortBuff);
                fileHeader.setVersionMadeBy(
                        Raw.readShortLittleEndian(shortBuff, 0));

                //version needed to extract
                readIntoBuff(zip4jRaf, shortBuff);
                fileHeader.setVersionNeededToExtract(
                        Raw.readShortLittleEndian(shortBuff, 0));
                ...
                fileHeader.setCompressedSize(
                        Raw.readLongLittleEndian(longBuff, 0));

                //uncompressed size
                readIntoBuff(zip4jRaf, intBuff);
                longBuff = getLongByteFromIntByte(intBuff);
                fileHeader.setUncompressedSize(
                        Raw.readLongLittleEndian(longBuff, 0));
                 ...
                 * (Raw.readIntLittleEndian(intBuff, 0) & 0xFFFFFFFFL)
                 * + zip4jRaf.getStart());
                 */
                longBuff = getLongByteFromIntByte(intBuff);
                fileHeader.setOffsetLocalHeader(
                        Raw.readLongLittleEndian(longBuff, 0) & 0xFFFFFFFFL);
                ...
                if (fileNameLength > 0) {
                    ...
                    if (Zip4jUtil.isStringNotNullAndNotEmpty(
                            zipModel.getFileNameCharset())) {
                        fileName = new String(
                                fileNameBuf, zipModel.getFileNameCharset());
                    } else {
                        fileName = Zip4jUtil.decodeFileName(fileNameBuf,
                                fileHeader.isFileNameUTF8Encoded());
                    }

                    if (fileName == null) {
                        throw new ZipException("fileName is null when reading "
                                + "central directory");
                    }
                    ...
                    fileHeader.setDirectory(fileName.endsWith("/")
                            || fileName.endsWith("\\"));
    ...
    private void readAndSaveExtraDataRecord(FileHeader fileHeader)
        ...
        if (zip4jRaf == null) {
            throw new ZipException("invalid file handler when trying to read "
                    + "extra data record");
        }
    ...
    private void readAndSaveExtraDataRecord(LocalFileHeader localFileHeader)
        ...
        localFileHeader.setExtraDataRecords(
                readExtraDataRecords(extraFieldLength));
     ...
     * @throws ZipException
     */
    private ArrayList readExtraDataRecords(int extraFieldLength)
            throws ZipException {
        ...
        try {
            ...
            while (counter < extraFieldLength) {
                ...
                counter = counter + 2;
                int sizeOfRec = Raw.readShortLittleEndian(
                        extraFieldBuf, counter);
                ...
                if (sizeOfRec > 0) {
                    ...
                    System.arraycopy(
                            extraFieldBuf, counter, data, 0, sizeOfRec);
    ...
    private Zip64EndCentralDirLocator readZip64EndCentralDirLocator()
        ...
        try {
            ...
            readIntoBuff(zip4jRaf, intBuff);
            zip64EndCentralDirLocator.setTotNumberOfDiscs(
                    Raw.readIntLittleEndian(intBuff, 0));
    ...
    private Zip64EndCentralDirRecord readZip64EndCentralDirRec()
        ...
        if (offSetStartOfZip64CentralDir < 0) {
            throw new ZipException("invalid offset for start of end of central "
                    + "directory record");
        }
        ...
        try {
            ...
            if (signature != InternalZipConstants.ZIP64ENDCENDIRREC) {
                throw new ZipException("invalid signature for zip64 end of "
            ...
            //version made by
            readIntoBuff(zip4jRaf, shortBuff);
            zip64EndCentralDirRecord.setVersionMadeBy(
                    Raw.readShortLittleEndian(shortBuff, 0));

            //version needed to extract
            readIntoBuff(zip4jRaf, shortBuff);
            zip64EndCentralDirRecord.setVersionNeededToExtract(
                    Raw.readShortLittleEndian(shortBuff, 0));

            //number of this disk
            readIntoBuff(zip4jRaf, intBuff);
            zip64EndCentralDirRecord.setNoOfThisDisk(
                    Raw.readIntLittleEndian(intBuff, 0));
            ...
            //size of the central directory
            readIntoBuff(zip4jRaf, longBuff);
            zip64EndCentralDirRecord.setSizeOfCentralDir(
                    Raw.readLongLittleEndian(longBuff, 0));
            ...
            long extDataSecSize = zip64EndCentralDirRecord
                    .getSizeOfZip64EndCentralDirRec() - 44;
            if (extDataSecSize > 0) {
                byte[] extDataSecRecBuf = new byte[(int) extDataSecSize];
                readIntoBuff(zip4jRaf, extDataSecRecBuf);
                zip64EndCentralDirRecord
                        .setExtensibleDataSector(extDataSecRecBuf);
            }
    ...
    private void readAndSaveZip64ExtendedInfo(FileHeader fileHeader)
            throws ZipException {
        if (fileHeader == null) {
            throw new ZipException("file header is null in reading Zip64 "
                    + "Extended Info");
        }

        if (fileHeader.getExtraDataRecords() == null
                || fileHeader.getExtraDataRecords().size() <= 0) {
            return;
        }
        ...
        if (zip64ExtendedInfo != null) {
            ...
            if (zip64ExtendedInfo.getUnCompressedSize() != -1) {
                fileHeader.setUncompressedSize(
                        zip64ExtendedInfo.getUnCompressedSize());
            }

            if (zip64ExtendedInfo.getCompressedSize() != -1) {
                fileHeader.setCompressedSize(
                        zip64ExtendedInfo.getCompressedSize());
            }

            if (zip64ExtendedInfo.getOffsetLocalHeader() != -1) {
                fileHeader.setOffsetLocalHeader(
                        zip64ExtendedInfo.getOffsetLocalHeader());
            }

            if (zip64ExtendedInfo.getDiskNumberStart() != -1) {
                fileHeader.setDiskNumberStart(
                        zip64ExtendedInfo.getDiskNumberStart());
            }
        }
    }
    ...
    private void readAndSaveZip64ExtendedInfo(LocalFileHeader localFileHeader)
        ...
        if (localFileHeader.getExtraDataRecords() == null
                || localFileHeader.getExtraDataRecords().size() <= 0) {
            return;
        }
     ...
     * @throws ZipException
     */
    private Zip64ExtendedInfo readZip64ExtendedInfo(ArrayList extraDataRecords,
        ...
        for (int i = 0; i < extraDataRecords.size(); i++) {
            ExtraDataRecord extraDataRecord
                    = (ExtraDataRecord) extraDataRecords.get(i);
            ...
            if (extraDataRecord.getHeader() == 0x0001) {
                ...
                if (((unCompressedSize & 0xFFFF) == 0xFFFF)
                        && counter < extraDataRecord.getSizeOfData()) {
                ...
                if (((compressedSize & 0xFFFF) == 0xFFFF)
                        && counter < extraDataRecord.getSizeOfData()) {
                    System.arraycopy(byteBuff, counter, longByteBuff, 0, 8);
                    long val = Raw.readLongLittleEndian(longByteBuff, 0);
                ...
                if (((offsetLocalHeader & 0xFFFF) == 0xFFFF)
                        && counter < extraDataRecord.getSizeOfData()) {
                    System.arraycopy(byteBuff, counter, longByteBuff, 0, 8);
                    long val = Raw.readLongLittleEndian(longByteBuff, 0);
                ...
                if (((diskNumberStart & 0xFFFF) == 0xFFFF)
                        && counter < extraDataRecord.getSizeOfData()) {
    ...
    private void setFilePointerToReadZip64EndCentralDirLoc()
        ...
        try {
            ...
            do {
                zip4jRaf.seek(pos--);
            } while (Raw.readLeInt(zip4jRaf, ebs)
                    != InternalZipConstants.ENDSIG);
            ...
            //FIXME Donot harcorde these values. Make use of ZipConstants
    ...
    public LocalFileHeader readLocalFileHeader(FileHeader fileHeader)
        ...
        try {
            ...
            if (fileNameLength > 0) {
                byte[] fileNameBuf = new byte[fileNameLength];
                readIntoBuff(zip4jRaf, fileNameBuf);
                String fileName = Zip4jUtil.decodeFileName(
                        fileNameBuf, localFileHeader.isFileNameUTF8Encoded());

                if (fileName == null) {
                    throw new ZipException("file name is null, cannot assign "
                            + "file name to local file header");
                }
    ...
    private void readAndSaveAESExtraDataRecord(FileHeader fileHeader)
        ...
        AESExtraDataRecord aesExtraDataRecord
                = readAESExtraDataRecord(fileHeader.getExtraDataRecords());
    ...
    private void readAndSaveAESExtraDataRecord(LocalFileHeader localFileHeader)
        ...
        AESExtraDataRecord aesExtraDataRecord
                = readAESExtraDataRecord(localFileHeader.getExtraDataRecords());
        if (aesExtraDataRecord != null) {
     ...
     * @throws ZipException
     */
    private AESExtraDataRecord readAESExtraDataRecord(
            ArrayList extraDataRecords) throws ZipException {
        ...
        for (int i = 0; i < extraDataRecords.size(); i++) {
            ...
            if (extraDataRecord.getHeader() == InternalZipConstants.AESSIG) {
                ...
                aesExtraDataRecord.setVersionNumber(
                        Raw.readShortLittleEndian(aesData, 0));
                ...
                aesExtraDataRecord.setCompressionMethod(
                        Raw.readShortLittleEndian(aesData, 5));
     ...
     * @throws ZipException
     */
    private byte[] readIntoBuff(RandomAccessFile zip4jRaf, byte[] buf)
            throws ZipException {
        try {
            if (zip4jRaf.read(buf, 0, buf.length) != -1) {
                return buf;
            } else {
                throw new ZipException("unexpected end of file when reading "
                        + "short buff");
            }
        } catch (IOException e) {
    ...
    private byte[] getLongByteFromIntByte(byte[] intByte) throws ZipException {
        if (intByte == null) {
            throw new ZipException("input parameter is null, cannot expand to "
                    + "8 bytes");
        }

        if (intByte.length != 4) {
            throw new ZipException("invalid byte length, cannot expand to 8 "
                    + "bytes");
        }

        byte[] longBuff = {intByte[0], intByte[1], intByte[2], intByte[3],
            0, 0, 0, 0};
        return longBuff;
    }
}
